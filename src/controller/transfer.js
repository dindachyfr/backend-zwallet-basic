...
const transfer = async(req,res,next)=>{	//.post('/transfer'
	try{
	const {sender_wallet_id, receiver_wallet_id, amount, notes} = req.body //masuk ke middleware buat cek saldo>amount
...
const cancelTransfer = async(req,res,next) => {		//.delete('/transfer/cancel/:id)
    try{
        ...
        const result = await transferModel.delTransaction(record_no)	//buat delete from transaction where id = record_no
