...
const getUsersFiltered = ({ filter }) => {
  return new Promise((resolve, reject) => {
    connection.query(`SELECT users.id, users.name, users.phone_number, users.email, wallet.id AS wallet_id, wallet.balance FROM users JOIN wallet ON (wallet.user_id = users.id) WHERE name LIKE '%${filter}%'`, (error, result) => {
      if (!error) {
        resolve(result);
      } else {
        reject(error);
